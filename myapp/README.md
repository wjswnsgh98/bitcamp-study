...
- BoardHandler 클래스 복제하여 독서록 게시판 추가
- 클래스 코드 복제와 문제점 확인

## 15. 인스턴스 필드와 인스턴스 메서드 활용

- BoardHandler 클래스 인스턴스 필드 및 메서드 적용
- 향후 확장성을 고려하여 MemberHandler 클래스에도 인스턴스 필드와 인스턴스 메서드 적용
  - 그래서 실무에서는 대부분의 클래스가 인스턴스 필드와 인스턴스 메서드로 구성된다.
- 향후 확장성을 고려하여 Prompt 클래스에도 인스턴스 필드와 인스턴스 메서드 적용
  - 생성자 도입: Scanner 사용할 입력 도구를 지정할 수 있게 한다.
- 의존 객체 주입의 개념과 구현
  - 생성자를 통해 Prompt 객체를 Handler 주입
...
## 24. 제너릭을 사용하여 타입을 파라미터로 다루기
  ...
  - ArrayList, LinkedList, Stack, Queue에 제너릭 적용하기
...
## 39. Reflection API를 활용하여 DAO메서드 호출을 자동화하기
